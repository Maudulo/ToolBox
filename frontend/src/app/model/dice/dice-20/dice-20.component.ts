...
export class Dice20Component implements AfterViewInit, OnDestroy {
  ...
  private checkIfDiceStopped() {
    ...
    const stopped = this.diceBodies.every(d =>
      d.velocity.length() < 0.05 && d.angularVelocity.length() < 0.05 && d.position.y < 2
    );
    ...
    if (stopped) {
      ...
      console.log('🎲 D20 Résultats :', results, '→ Somme totale :', sum);
    }
  }
