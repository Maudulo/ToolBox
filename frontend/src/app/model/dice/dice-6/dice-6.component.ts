...
export class Dice6Component implements AfterViewInit, OnDestroy {
  ...
  public diceCount = 3;            // nombre de dés à lancer
  ...
  private initPhysics() {
    ...
    // Sol
    const groundShape = new CANNON.Box(new CANNON.Vec3(zoneSize / 2, 0.05, zoneSize / 2));
    ...
    createWallBox(0, wallHeight / 2, zoneSize / 2, zoneSize, wallHeight, wallThickness);   // Nord
    createWallBox(0, wallHeight / 2, -zoneSize / 2, zoneSize, wallHeight, wallThickness);  // Sud
    createWallBox(zoneSize / 2, wallHeight / 2, 0, wallThickness, wallHeight, zoneSize);   // Est
    createWallBox(-zoneSize / 2, wallHeight / 2, 0, wallThickness, wallHeight, zoneSize);  // Ouest

    // Plafond
    const ceilingShape = new CANNON.Box(new CANNON.Vec3(zoneSize / 2, 0.05, zoneSize / 2));
  ...
  private createDice(position: CANNON.Vec3): CANNON.Body {
    ...
    const materials = [
      new THREE.MeshStandardMaterial({ map: loader.load('assets/dice_6/faces-1.png') }),
      new THREE.MeshStandardMaterial({ map: loader.load('assets/dice_6/faces-6.png') }),
      new THREE.MeshStandardMaterial({ map: loader.load('assets/dice_6/faces-3.png') }),
      new THREE.MeshStandardMaterial({ map: loader.load('assets/dice_6/faces-4.png') }),
      new THREE.MeshStandardMaterial({ map: loader.load('assets/dice_6/faces-5.png') }),
      new THREE.MeshStandardMaterial({ map: loader.load('assets/dice_6/faces-2.png') }),
    ];
